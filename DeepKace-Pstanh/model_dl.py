...
class ClippedLeakyTanh(nn.Module):
    def __init__(self, alpha=0.5, beta =0.5,a=-1.0, b=1.0):
        super(ClippedLeakyTanh, self).__init__()
        self.alpha = torch.nn.Parameter(torch.tensor(alpha), requires_grad=True)
        self.beta = torch.nn.Parameter(torch.tensor(beta), requires_grad=True)
        self.a = a
        self.b = b
    ...
    def forward(self, x):
        return self.alpha * torch.clamp(x, self.a, self.b) + self.beta * torch.clamp(torch.tanh(x), self.a, self.b)


'''
class ClippedLeakyTanhswish(nn.Module):
    def __init__(self, alpha=1.0, beta=0.5, a=-4.0, b=1.0):
        super(ClippedLeakyTanhswish, self).__init__()
        self.alpha = torch.nn.Parameter(torch.tensor(alpha), requires_grad=True)
        self.beta = torch.nn.Parameter(torch.tensor(beta), requires_grad=True)
        self.a = a
        self.b = b
    ...
    def forward(self, x):
        return self.alpha * torch.clamp(x*torch.sigmoid(x), self.a, self.b) + self.beta * torch.clamp(torch.exp(x)*torch.tanh(x), self.a, self.b)
'''
...
class Pstanh(nn.Module):
    ...
    def forward(self, x):
        #return  torch.clamp(self.alpha*x*torch.sigmoid(x)+ self.beta*torch.exp(x)*torch.tanh(x) , self.a, self.b)
        #return torch.where(x<0,self.alpha * x * torch.sigmoid(x) + self.beta * torch.exp(x) * torch.tanh(x),torch.tanh(x)*(self.beta+(self.alpha)/2))
...
class Result(nn.Module):
    def __init__(self,num):
        ...
        #self.act_fun = Mish()
        #self.act_fun = Swish()
        #self.act_fun = LeakyTanhswish()
        #self.act_fun = ClippedLeakyTanh()
        self.act_fun = Pstanh()
        #self.act_fun = ClippedLeakyTanhmish()
